 ...
 * <p>
 * 基于一个给定设备的参数和设备屏幕密度智能计算出需要的缓存大小
 */
public final class MemorySizeCalculator {
    ...
    private MemorySizeCalculator(MemorySizeCalculator.Builder builder) {
        ...
        /**
         * 理想情况下：图片池所占内存（bitmapPoolSize）+ 缓存所占内存（memoryCacheSize）+
         * 数组池所占内存（arrayPoolSize） = 设备进程可用内存 * 分配系数（默认有0.33和0.4）
         */
    ...
    /**
     * 根据设备是否为低内存设备来设置可用的最大内存数（默认情况下，低内存设备的内存缓存大小为
     * 当前进程可支配的最大内存*0.33，反之乘以0.4）
     */
    private static int getMaxSize(ActivityManager activityManager, float maxSizeMultiplier,
                                  float lowMemoryMaxSizeMultiplier) {
        /**
         * 通过代码查看每个进程可用的最大内存，即heapgrowthlimit值（单位：MB）
         */
        final int memoryClassBytes = activityManager.getMemoryClass() * 1024 * 1024;
        /**
         * 判断设备是否为Heap Size低于512M或者屏幕大小<=800*480的低内存设备
         */
    ...
    /**
     * 通常判断的是Heap Size低于512M或者屏幕大小<=800*480的设备
     */
