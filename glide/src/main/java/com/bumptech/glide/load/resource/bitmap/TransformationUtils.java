...
public final class TransformationUtils {
    ...
    public static Bitmap centerCrop(@NonNull BitmapPool pool, @NonNull Bitmap inBitmap, int width,
        ...
        final float scale;// 缩放系数
        float dx = 0, dy = 0;
        Matrix m = new Matrix();
        /**
         * W1*H2>W2*H1 => H2/H1>W2/W1 即是说高度被保留得更多，而此时采用目标高度值作为缩放准则，
         * 那么要想保持宽高比缩放后不变，那么目标宽度值肯定就比采用高度值作为缩放准则后得出的目标
         * 宽度要小，此时，dx即为这个偏差值的1/2
         */
     ...
     * <p>
     * 将图片裁剪成圆形状图片，圆形图片直径是传参传参宽度和高度之间较小值
     *
     * @param pool       The BitmapPool obtain a bitmap from.
     * @param inBitmap   The Bitmap to resize.
