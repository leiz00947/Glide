...
class DecodeJob<R> implements DataFetcherGenerator.FetcherReadyCallback,
    ...
    /**
     * @param diskCacheProvider {@link Engine.LazyDiskCacheProvider}
     * @param pool              {@link Engine.DecodeJobFactory#pool}
     */
     ...
     * <p>
     * 若返回{@code true}，表示通过磁盘缓存解码
     * <p>
     * 默认{@link #diskCacheStrategy}为{@link DiskCacheStrategy#AUTOMATIC}，
     * 这种情况下，{@link #getNextStage(Stage)}返回值为{@link Stage#RESOURCE_CACHE}，
     * 那么，该方法返回值为{@code true}
     */
    ...
    /**
     * 完整的运行流程应该是：<ul>
     * <li>{@link #stage}为{@link Stage#RESOURCE_CACHE}，此时{@link #currentGenerator}为
     * {@link ResourceCacheGenerator}的实例，执行{@link ResourceCacheGenerator#startNext()} ,
     * 若未执行到{@link DataFetcher#loadData(Priority, DataFetcher.DataCallback)},则执行下一步</li>
     * <li>此时{@link #stage}为{@link Stage#DATA_CACHE}，此时{@link #currentGenerator}为
     * {@link DataCacheGenerator}的实例，执行{@link DataCacheGenerator#startNext()}，同上一步</li>
     * <li>此时{@link #stage}为{@link Stage#SOURCE}，此时{@link #currentGenerator}为
     * {@link SourceGenerator}的实例，那么回调{@link EngineJob#reschedule(DecodeJob)}，此时
     * {@link #runReason}为{@link RunReason#SWITCH_TO_SOURCE_SERVICE}，继续执行
     * {@link #runGenerators()}，若{@link SourceGenerator#startNext()}返回{@code false}，那么回调
     * {@link #notifyFailed()}，反之，会在相应的
     * {@link DataFetcher#loadData(Priority, DataFetcher.DataCallback)}中回调
     * {@link #onDataFetcherReady(Key, Object, DataFetcher, DataSource, Key)}进而调用
     * {@link #decodeFromRetrievedData()}进行解码操作</li></ul>
     */
    @Override
    public void run() {
     ...
     * <p>
     * 解码数据阶段，解码阶段流程：<ul>
     * <li>{@link Stage#INITIALIZE}</li>
     * <li>{@link Stage#RESOURCE_CACHE}</li>
     * <li>{@link Stage#DATA_CACHE}</li>
     * <li>{@link Stage#SOURCE}</li>
     * <li>{@link Stage#ENCODE}</li>
     * <li>{@link Stage#FINISHED}</li></ul>
     */
    private enum Stage {
        /**
         * The initial stage.
